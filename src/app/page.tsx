"use client";

import { useState } from "react";
import { PostForm } from "@/components/post-form";
import { PostPreview } from "@/components/post-preview";
import { TrendDisplay } from "@/components/trend-display";
import { ThreadsAnalyzer } from "@/components/threads-analyzer";
import { ViralExplorer } from "@/components/viral-explorer";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { Sparkles, BarChart3, Globe } from "lucide-react";

export default function Home() {
  const [activeTab, setActiveTab] = useState("generator");

  return (
    <div className="relative min-h-screen overflow-hidden">
      {/* 背景裝飾球體 */}
      <div className="bg-orb bg-orb-1" />
      <div className="bg-orb bg-orb-2" />
      <div className="bg-orb bg-orb-3" />

      {/* Header */}
      <header className="relative z-50 border-b border-white/5">
        <div className="absolute inset-0 bg-gradient-to-r from-white/[0.02] to-transparent backdrop-blur-xl" />
        <div className="container relative mx-auto px-4 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {/* Logo */}
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-r from-primary to-purple-500 blur-xl opacity-50" />
                <div className="relative gradient-primary rounded-2xl p-3 text-white shadow-lg shadow-primary/25">
                  <svg
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-7 w-7"
                  >
                    <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.59 12c.025 3.086.718 5.496 2.057 7.164 1.43 1.783 3.631 2.698 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.96-.065-1.182.408-2.256 1.33-3.022.88-.73 2.082-1.123 3.479-1.142.987-.014 1.95.125 2.716.328l.026-.013c-.007-.04-.015-.08-.024-.12-.173-.858-.49-1.504-.944-1.923-.519-.479-1.25-.73-2.172-.747h-.008c-.746.013-1.697.197-2.478.862l-1.305-1.543c1.166-.987 2.575-1.443 3.782-1.454h.013c1.424.022 2.595.475 3.479 1.346.758.748 1.266 1.749 1.512 2.975.077.385.132.785.164 1.19 1.21.511 2.18 1.29 2.817 2.293.893 1.407 1.033 3.155.394 4.929-.91 2.522-3.243 4.188-6.85 4.205h-.016zm-1.055-9.96c-1.097.013-1.995.27-2.538.727-.493.413-.734.933-.7 1.505.036.628.371 1.147.944 1.461.582.318 1.325.448 2.097.412 1.105-.058 1.948-.476 2.507-1.241.322-.442.523-.947.634-1.559-.71-.178-1.683-.326-2.944-.305z" />
                  </svg>
                </div>
              </div>
              {/* Title */}
              <div>
                <h1 className="text-2xl font-bold tracking-tight">
                  <span className="gradient-text">Threads</span>
                  <span className="text-foreground"> 貼文產生器</span>
                </h1>
                <p className="text-sm text-muted-foreground">
                  AI 驅動的高流量貼文創作工具
                </p>
              </div>
            </div>

            {/* 狀態指示 */}
            <div className="hidden md:flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <span className="text-xs text-muted-foreground">AI 已就緒</span>
            </div>
          </div>
        </div>
      </header>

      {/* Main Content */}
      <main className="relative z-10 container mx-auto px-4 py-10">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          {/* Tab 導航 */}
          <div className="flex justify-center">
            <TabsList className="grid w-full max-w-lg grid-cols-3">
              <TabsTrigger value="generator" className="gap-2">
                <Sparkles className="w-4 h-4" />
                <span className="hidden sm:inline">貼文產生</span>
                <span className="sm:hidden">產生</span>
              </TabsTrigger>
              <TabsTrigger value="viral" className="gap-2">
                <Globe className="w-4 h-4" />
                <span className="hidden sm:inline">國外爆款</span>
                <span className="sm:hidden">爆款</span>
              </TabsTrigger>
              <TabsTrigger value="analyzer" className="gap-2">
                <BarChart3 className="w-4 h-4" />
                <span className="hidden sm:inline">帳號分析</span>
                <span className="sm:hidden">分析</span>
              </TabsTrigger>
            </TabsList>
          </div>

          {/* Generator Tab */}
          <TabsContent value="generator" className="mt-8 animate-fade-in">
            <div className="grid gap-8 lg:grid-cols-[420px_1fr]">
              {/* 左側：熱門話題 + 輸入表單 */}
              <aside className="space-y-6 lg:sticky lg:top-8 lg:self-start">
                <TrendDisplay />
                <PostForm />
              </aside>

              {/* 右側：結果展示 */}
              <section className="min-h-[400px]">
                <PostPreview />
              </section>
            </div>
          </TabsContent>

          {/* Viral Explorer Tab */}
          <TabsContent value="viral" className="mt-8 animate-fade-in">
            <div className="max-w-4xl mx-auto">
              <ViralExplorer />
            </div>
          </TabsContent>

          {/* Analyzer Tab */}
          <TabsContent value="analyzer" className="mt-8 animate-fade-in">
            <div className="max-w-4xl mx-auto">
              <ThreadsAnalyzer />
            </div>
          </TabsContent>
        </Tabs>
      </main>

      {/* Footer */}
      <footer className="relative z-10 border-t border-white/5 mt-auto">
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.02] to-transparent" />
        <div className="container relative mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">
              Threads Post Generator - AI 驅動的社群貼文創作工具
            </p>
            <div className="flex items-center gap-6 text-xs text-muted-foreground">
              <span className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                Powered by Perplexity AI
              </span>
              <span className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-500" />
                Meta Graph API
              </span>
            </div>
          </div>
        </div>
      </footer>
    </div>
  );
}
